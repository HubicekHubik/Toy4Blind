...
K_MSGQ_DEFINE(imu_msgq, sizeof(float) * 3, 32, 4);
/**/
                                    ...
                                    const struct sensor_trigger *trig) 
{
    ...
    /* Fetch sensor data – to je OK v ISR, LSM6DSL je rychlý */
    if (sensor_sample_fetch_chan(dev, SENSOR_CHAN_ACCEL_XYZ) != 0) {
        return; /* V ISR nikdy nedělat printk ani log */
    }
    ...
    sample_buf[0] = sensor_value_to_double(&ax);
    sample_buf[1] = sensor_value_to_double(&ay);
    sample_buf[2] = sensor_value_to_double(&az);

    /* Poslat data do queue – jen pokud je místo */
	//printk("Acel data Ax:%f Ay:%f Az:%f\n", sample_buf[0], sample_buf[1], sample_buf[2]);
...
int playaudio(const struct device *dev_i2s, const char *path); // plays the recieved audio data using i2s checks bitrate
